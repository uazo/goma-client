...
class FakeFlags : public CompilerFlags {
 public:
  FakeFlags(const std::vector<std::string>& args, const std::string& cwd);

  // Returns the compiler family name.
  std::string compiler_name() const override { return "fake"; }
  ...
  static std::string GetCompilerName(absl::string_view arg) { return "fake"; }
};
